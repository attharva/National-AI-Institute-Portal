...
function reportDataUpdate (id, title, year, venue, authors){
    ...
    fetch('/member/updateResearchReportData', {
        method: 'POST',
        body: formData
    })
...
function reportFileUpdate(id, updated_files, newFiles){
    ...
    fetch('/member/updateResearchReportFileData', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const fetchAllEventsData = () => {
        const fetchUrl = `/member/getResearchReportData`;
        ...
        fetch(fetchUrl, {
            ...
            .then(data => {
                if (data.hasOwnProperty("reportData") && Array.isArray(data.reportData)) {
                    ...
                    populateDropdown(statusFilterDropdown, getUniqueValues(allReportData, 'title'));
    ...
    const displayData = (data) => {
        ...
        firstAnchor.addEventListener("click",  async function () {
            ...
            <input type="number" id="year" name="year" placeholder="YYYY" min="1900" max="2100" class="form-control bg-light border-0 small" required value="${item.year}"><br><br>
                
            <label for="venue">Venue:<span style="color: red;">*</span></label>
            <input id="venue" name="venue" class = "form-control bg-light border-0 small" required value="${item.venue}"></textarea><br><br>
            
            <label for="authors">Authors:<span style="color: red;">*</span></label>
            <input id="authors" name="authors" class = "form-control bg-light border-0 small" required value="${item.authors}"></textarea><br><br>
            ...
            didOpen: () => {
                ...
                if (result.isConfirmed) {
                    ...
                    let updated_year =  document.getElementById('year').value
                    var updated_venue = document.getElementById('venue').value;
                    var updated_authors = document.getElementById('authors').value;
                    
                    let new_file = document.getElementById('fileUpload').files
                    let updated_files =  report_file_ids[item.id]
                    // console.log(item.id, updated_title, updated_year, updated_venue, updated_authors )
                    reportDataUpdate(item.id, updated_title, updated_year, updated_venue, updated_authors );
