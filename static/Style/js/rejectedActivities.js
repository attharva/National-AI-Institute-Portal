document.addEventListener("DOMContentLoaded", function() {
    ...
    fetch('/admin/getRejectedActivities', {
        method: 'GET'
    })
    ...
    .then(data => {
        ...
        if (data.hasOwnProperty("newsData") && Array.isArray(data.newsData)) {
            data.newsData.forEach((item, index) => {
                ...
                secondAnchor.classList.add("btn", "btn-warning", "btn-circle");

                // Create the second icon element
                const secondIcon = document.createElement("i");
                secondIcon.classList.add("fas", "fa-exclamation-triangle");
                secondAnchor.appendChild(secondIcon);
                secondAnchor.title = "Pending";
                        ...
                        if (result.isConfirmed) {
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    Swal.fire(
                                        'Approved!',
                                        'Successfully added to Approved Activities',
                                        'success'
                                    );
                                } else {
                                    Swal.fire({
