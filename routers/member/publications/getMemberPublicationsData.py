...
memberGetMemberPublicationsData = APIRouter()

@memberGetMemberPublicationsData.get("/member/getMemberPublicationsData")
async def get_eventsData(request: Request):
    try:
        ...
        try:
            ...
            query = f"SELECT p.name, CONCAT(m.firstName, ' ', m.lastName) as AuthorName, p.year, p.venue, p.citedBy_Scholar, p.citedBy_ACM, p.citedBy_OpenAlex, p.citedBy_IEEE, p.publicationId, p.authors FROM publications p, members m WHERE p.authorId = m.id AND p.authorId not in ({memberID}) AND p.status = 1 ORDER BY year DESC;"
