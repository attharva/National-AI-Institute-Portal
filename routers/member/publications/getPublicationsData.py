...
memberGetPublicationsData = APIRouter()

@memberGetPublicationsData.get("/member/getPublicationsData")
async def get_eventsData(request: Request):
    try:
        ...
        try:
            ...
            query = f"SELECT name, year, venue, citedBy_Scholar, citedBy_ACM, citedBy_OpenAlex, citedBy_IEEE, publicationId, authors FROM publications WHERE authorId = {memberID} and status = 1 ORDER BY year DESC"
