...
def is_valid_jwt(jwt_cookie):
    try:
        ...
        return False
memberMemberPublicationsView = APIRouter()

@memberMemberPublicationsView.get("/member/memberpublications/view")
async def member_events(request: Request):
    try:
        jwt_cookie = request.cookies.get("access_token")
        if is_valid_jwt(jwt_cookie):
            html_file_path = Path("./routers/member/views/memberpublications_view.html")
