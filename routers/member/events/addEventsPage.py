...
def is_valid_jwt(jwt_cookie):
    try:
        ...
        return False
memberEventsAdd = APIRouter()

@memberEventsAdd.get("/member/events/add")
async def member_news(request: Request):
    try:
        jwt_cookie = request.cookies.get("access_token")
        if is_valid_jwt(jwt_cookie):
            html_file_path = Path("./routers/member/views/events_add.html")
