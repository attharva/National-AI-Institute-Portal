...
adminHidePublicationsData = APIRouter()
class PublicationIdsClass(BaseModel):
    publicationIds: List[int]
    comments: str

@adminHidePublicationsData.post("/admin/hidePublicationsData")
async def get_eventsData(publicationIdsObj: PublicationIdsClass, request: Request):
    try:
        ...
        try:
            ...
            query = f"Update publications set status = 0, comments = '{comments}', modifiedBy = '{adminID}' WHERE publicationId in ({ids_string});"
