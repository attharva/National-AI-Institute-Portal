...
adminGetNewPublicationsData = APIRouter()

@adminGetNewPublicationsData.get("/admin/getNewPublicationsData")
async def get_eventsData(request: Request):
    try:
        ...
        try:
            ...
            query = f"SELECT p.name, CONCAT(m.firstName, ' ', m.lastName) as AuthorName, p.year, p.venue, p.citedBy_Scholar, p.citedBy_ACM, p.citedBy_OpenAlex, p.citedBy_IEEE, p.publicationId, p.authors FROM publications p, members m WHERE p.authorId = m.id AND p.status = 1 AND date(p.createdDate) >= date(NOW() - interval 7 day) ORDER BY year DESC;"
