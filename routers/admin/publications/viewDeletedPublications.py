...
def is_valid_jwt(jwt_cookie):
    try:
        ...
        return False
adminDeletedPublicationsView = APIRouter()

@adminDeletedPublicationsView.get("/admin/deletedpublications/view")
async def admin_events(request: Request):
    try:
        jwt_cookie = request.cookies.get("access_token")
        if is_valid_jwt(jwt_cookie):
            html_file_path = Path("./routers/admin/views/deletedpublications_view.html")
