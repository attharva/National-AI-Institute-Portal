...
def is_valid_jwt(jwt_cookie):
    try:
        ...
        return False
admindeadlineview = APIRouter()

@admindeadlineview.get("/admin/deadline/view")
async def admin_viewDeadlinePage(request: Request):
    try:
        jwt_cookie = request.cookies.get("access_token")
        if is_valid_jwt(jwt_cookie):
            html_file_path = Path("./routers/admin/views/deadline_view.html")
