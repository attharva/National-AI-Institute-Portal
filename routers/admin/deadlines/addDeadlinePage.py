...
def is_valid_jwt(jwt_cookie):
    try:
        ...
        return False
admindeadlineadd = APIRouter()

@admindeadlineadd.get("/admin/deadline/add")
async def admin_deadline_add(request: Request):
    try:
        jwt_cookie = request.cookies.get("access_token")
        if is_valid_jwt(jwt_cookie):
            html_file_path = Path("./routers/admin/views/deadline_add.html")
